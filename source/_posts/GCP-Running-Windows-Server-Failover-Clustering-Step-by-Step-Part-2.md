...
整個範例其實展示的結果很簡單但對於企業內部再使用雲端服務的幫助則是非常大的，以往要自行建立這樣的叢集容錯機制是具有相當大的難度與複雜度，但如今透過 GCP 的指令與 Windows UI 上面的設定精靈，我們可以很快速地該項設定，提供網頁應用程式更好的高可用性(High availability)。

而這個機制也不只用於內網，其實對外公開網站也可以用其方法建立叢集處理，提供可用性與其效能!